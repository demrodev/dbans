...
public class H2Database implements DatabaseManager {
    ...
    public void init() throws SQLException {
        String file = plugin.getConfig().getString("database.h2.file", "data");
        String url = "jdbc:h2:" + plugin.getDataFolder().getAbsolutePath() + "/" + file + ";DB_CLOSE_DELAY=-1";
    ...
    public void updatePunishmentEndTime(String id, long newEndTime) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            // Вызов события изменения длительности
                ...
                try {
                    ...
                    if (em != null) {
                        PunishmentModifyEvent event = new PunishmentModifyEvent(
                                ...
                                null, null, oldEndTime, updated.getEndTime()
                        );
                        em.callEvent(event);
                    }
                } catch (Exception e) {
    ...
    public Punishment getActivePunishment(UUID playerUuid, PunishmentType type, String currentServer, String mode) {
        ...
        if ("sync_static".equalsIgnoreCase(mode)) {
            sql = "SELECT * FROM punishments WHERE player_uuid=? AND type=? AND active=TRUE AND server_name=? ORDER BY start_time DESC";
        } else {
            sql = "SELECT * FROM punishments WHERE player_uuid=? AND type=? AND active=TRUE ORDER BY start_time DESC";
        }
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<Punishment> getActivePunishments(UUID playerUuid, String currentServer, String mode) {
        ...
        String sql;
        if ("sync_static".equalsIgnoreCase(mode)) {
            sql = "SELECT * FROM punishments WHERE player_uuid=? AND active=TRUE AND server_name=? ORDER BY start_time DESC";
        } else {
    ...
    public void updatePunishmentReason(String id, String newReason) {
        // Сохраняем старую причину до обновления
        Punishment oldP = getPunishmentById(id);
    ...
    public void pardonPunishment(String id, String pardonedBy, String pardonReason) {
        // Получаем полный объект наказания из БД
        Punishment p = getPunishmentById(id);
        if (p == null) return;
    ...
    public boolean isJailed(UUID playerUuid) {
        ...
        return j != null && j.isActive() && !j.isExpired();
    }

    @Override
    public List<JailPunishment> getAllActiveJails() {
        List<JailPunishment> list = new ArrayList<>();
        String sql = "SELECT * FROM jail_punishments WHERE active=TRUE";
        try (Connection conn = dataSource.getConnection();
             Statement st = conn.createStatement();
             ResultSet rs = st.executeQuery(sql)) {
    ...
    public List<JailPunishment> getExpiredActiveJails() {
        List<JailPunishment> list = new ArrayList<>();
        String sql = "SELECT * FROM jail_punishments WHERE active=TRUE AND end_time IS NOT NULL AND end_time <= ?";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setLong(1, System.currentTimeMillis());
            ResultSet rs = ps.executeQuery();
    ...
    public List<Punishment> getActivePunishmentsIncludingJail(UUID playerUuid, String currentServer, String mode) {
        ...
        String sql1;
        if ("sync_static".equalsIgnoreCase(mode)) {
            sql1 = "SELECT * FROM punishments WHERE player_uuid=? AND active=TRUE AND server_name=?";
        } else {
            sql1 = "SELECT * FROM punishments WHERE player_uuid=? AND active=TRUE";
        }
        try (Connection conn = dataSource.getConnection();
        ...
        String sql2;
        if ("sync_static".equalsIgnoreCase(mode)) {
            sql2 = "SELECT *, 'JAIL' as type FROM jail_punishments WHERE player_uuid=? AND active=TRUE AND server_name=?";
        } else {
            sql2 = "SELECT *, 'JAIL' as type FROM jail_punishments WHERE player_uuid=? AND active=TRUE";
        }
        try (Connection conn = dataSource.getConnection();
        ...
        String sql3;
        if ("sync_static".equalsIgnoreCase(mode)) {
            sql3 = "SELECT *, 'WARNING' as type FROM warnings WHERE player_uuid=? AND active=TRUE AND server_name=?";
        } else {
            sql3 = "SELECT *, 'WARNING' as type FROM warnings WHERE player_uuid=? AND active=TRUE";
        }
        try (Connection conn = dataSource.getConnection();
    ...
    public int getTotalPunishmentsCountByPlayer(String playerName) {
        String sql = "SELECT COUNT(*) FROM (SELECT 1 FROM punishments WHERE player_name = ? UNION ALL SELECT 1 FROM jail_punishments WHERE player_name = ? UNION ALL SELECT 1 FROM warnings WHERE player_name = ?)";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, playerName);
            ps.setString(2, playerName);
    ...
    public int getPunishmentsIssuedByPlayer(String issuerName) {
        String sql = "SELECT COUNT(*) FROM (SELECT 1 FROM punishments WHERE issuer_name = ? UNION ALL SELECT 1 FROM jail_punishments WHERE issuer_name = ? UNION ALL SELECT 1 FROM warnings WHERE issuer_name = ?)";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
